...
- 创建文件夹：`土壤（soil）`, `堆肥（Compost）`, `枯枝（Archive）`（根级共享）。
- **每个领域文件夹内自动生成标准结构（v2.7）**：`树干-{领域}.md`、`枝条（branches）/`、`叶片（leaves）/`、`花朵（Flowers）/`、`果实（Fruits）/`——花朵与果实归领域所有，堆肥全森林共享。
...
## 3. 日常操作流程
- **蓄土**：一切碎片丢进 `土壤（soil）`。
- **播种**：用 `种子模板（t-core-seed）` 创建领域种子（如“读书”），可同时多条；**立干时自动生成领域五结构**（树干/枝条/叶片/花朵/果实子目录）。
...
- **开花**：用 `问题模板（t-question）` 写下问题，运行 Work Buddy 命令开多花，**产出存入该领域 `花朵（Flowers）/`**。
- **结果评级**：
  - 小果直接手动评好/坏。
  - 大果由 Work Buddy 生成 4 个判定标准，确认后执行，执行后 AI 建议评级你确认。
  - **果实存入该领域 `果实（Fruits）/`**，好果模式写回树干，坏果教训回土壤。
- **修剪**：月底查看黄叶清单，处理长期无链接的土壤碎片；黄叶落叶后进入**全森林共享的 `堆肥（Compost）`**，任何领域开花时都可能调用（施肥）。
